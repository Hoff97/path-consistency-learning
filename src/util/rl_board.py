...
class RLBoard:
    ...
    def log(self, iteration, rewards, loss, lens, exploration_rate):
        ...
        self.writer.add_scalar('exploration_rate', avg_len, global_step=iteration)

        print(f'[{iteration+1}] Loss: {loss.item()}, Rewards: {avg_reward}, Len: {avg_len}, Exp: {exploration_rate}')
